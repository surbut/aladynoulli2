...
def analyze_primary_secondary_cancer(model, Y,disease_names, window_size=5):
    """
...
def analyze_disease_relationships(model, Y, disease_names, window_size=5):
    ...
    """
    # Convert disease names to list if it's a pandas Series/DataFrame
    if hasattr(disease_names, 'tolist'):
        disease_names = disease_names.tolist()
    elif hasattr(disease_names, 'iloc'):
