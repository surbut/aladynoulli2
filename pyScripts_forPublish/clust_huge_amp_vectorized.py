...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest(nn.Module):
    ...
    def compute_loss(self, event_times):
        ...
        # Handle both 1D [N] and 2D [N, D] input like fixedPhi version
        event_times_expanded = event_times_tensor.unsqueeze(-1)  # [N, 1] or [N, D, 1]
        time_grid = torch.arange(T, dtype=torch.long).unsqueeze(0).unsqueeze(0)  # [1, 1, T]
        mask_before_event = (time_grid < event_times_expanded).float()  # [N, 1, T] or [N, D, T]
        mask_at_event = (time_grid == event_times_expanded).float()  # [N, 1, T] or [N, D, T]
