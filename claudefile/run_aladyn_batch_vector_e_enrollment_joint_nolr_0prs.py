...
with G = sex + PCs only (no PRS).

Uses E_enrollment_full.pt (enrollment matrix) for training. Same as
run_aladyn_batch_vector_e_enrollment_joint_nolr.py but G is sex and PCs only
(no PRS columns). The resulting theta/AEX has no PRS effects
and is suitable for GWAS (no circularity from PRS in the phenotype).

This script:
- Uses E_enrollment_full.pt (same as prediction scripts)
- Performs JOINT ESTIMATION (trains phi, psi, and lambda together)
- G = [sex, PCs] only — no PRS
- Uses clust_huge_amp_vectorized_nolr.py — no lambda_reg
- AWS-compatible with environment variable defaults

Requirements (same as nolr):
- clust_huge_amp_vectorized_nolr.py
...
Usage:
    Local:  python run_aladyn_batch_vector_e_enrollment_joint_nolr_0prs.py --start_index 0 --end_index 10000
    AWS:    python run_aladyn_batch_vector_e_enrollment_joint_nolr_0prs.py --start_index 0 --end_index 10000 --data_dir /data --output_dir /results
"""
...
def load_model_essentials(base_path):
    ...
    print(f"  G shape: {G.shape} (PRS columns will be zeroed)")
...
def main():
    parser = argparse.ArgumentParser(description='Run Aladyn batch with joint estimation (enrollment E, NOLR, 0 PRS)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default=os.getenv('OUTPUT_DIR', '/Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_joint_estimation_batchrun_nolr_0prs'),
    ...
    print(f"\n{'='*60}")
    print(f"Running Aladyn batch (VECTORIZED, JOINT ESTIMATION, ENROLLMENT E, NOLR, 0 PRS):")
    print(f"  Samples {args.start_index} to {args.end_index}")
    print(f"  Using E_enrollment_full.pt (enrollment matrix)")
    print(f"  G = sex + PCs only (no PRS)")
    ...
    if args.include_pcs:
        ...
        G_with_sex = np.column_stack([sex_batch, pcs])
    print(f"G shape: {G_with_sex.shape} (sex + PCs only, no PRS)")
    ...
    print(f"\nInitializing model with K={args.K} clusters (VECTORIZED, JOINT, NOLR, 0 PRS)...")
    ...
    print(f"\nTraining model for {args.num_epochs} epochs (JOINT ESTIMATION, NOLR, 0 PRS)...")
    ...
    output_path = output_dir / f'enrollment_joint_model_VECTORIZED_nolr_0prs_W{args.W}_batch_{args.start_index}_{args.end_index}.pt'
    ...
    torch.save({
        ...
        'version': 'VECTORIZED_JOINT_ENROLLMENT_NOLR_0PRS',
