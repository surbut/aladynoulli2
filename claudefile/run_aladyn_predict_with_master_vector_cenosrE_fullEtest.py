#!/usr/bin/env python
"""
Prediction script for Aladyn model using FULL RETROSPECTIVE DATA with fixed phi/psi
...
- Uses FULL E matrix (all retrospective data, not just enrollment subset)
- Saves predictions (pi) for each batch
- Concatenates all batches at the end

This is similar to run_aladyn_batch but for enrollment predictions with full E matrix.
...
#master_for_fitting_pooled_correctedE.pt
nohup python run_aladyn_predict_with_master.py \
    ...
    --output_dir /Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_correctedE_vectorized/ \
    --max_batches 40 \
    > predict_correctedE_vectorized.log 2>&1 &



#master_for_fitting_pooled_correctedE.pt - USING FULL E MATRIX (all retrospective data)
# This is like run_aladyn_batch but for enrollment predictions with full E
nohup python run_aladyn_predict_with_master_vector_cenosrE_fullEtest.py \
    --trained_model_path /Users/sarahurbut/Library/CloudStorage/Dropbox-Personal/data_for_running/master_for_fitting_pooled_correctedE.pt \
...
def load_model_essentials(base_path, use_full_E=True):
    """Load all essential components
    
    Args:
        base_path: Path to data directory
        use_full_E: If True, use full retrospective E matrix (E_matrix.pt or E_matrix_corrected.pt)
                    If False, use enrollment-only E matrix
    """
    ...
    # Use full retrospective E matrix (all data) instead of enrollment-only
    if use_full_E:
        # Try corrected E first, fall back to regular E
        E_corrected_path = base_path + 'E_matrix_corrected.pt'
        #E_regular_path = base_path + 'E_matrix.pt'
        
        if Path(E_corrected_path).exists():
            E = torch.load(E_corrected_path, weights_only=False)
            print(f"  Using FULL retrospective E matrix (corrected): {E.shape}")
        elif Path(E_regular_path).exists():
            E = torch.load(E_regular_path, weights_only=False)
            print(f"  Using FULL retrospective E matrix (regular): {E.shape}")
        else:
            raise FileNotFoundError(f"Neither {E_corrected_path} nor {E_regular_path} found!")
    else:
        # Enrollment-only E (original behavior)
        E = torch.load(base_path + 'E_enrollment_full.pt', weights_only=False)
        print(f"  Using enrollment-only E matrix: {E.shape}")
...
def main():
    ...
    # Load data - use full E matrix (all retrospective data)
    Y, E, G, essentials = load_model_essentials(args.data_dir, use_full_E=True)
