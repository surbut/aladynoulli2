...
This script:
- Uses FIXED phi and psi from a trained model
...
Usage:
    Local:  python run_aladyn_predict.py --trained_model_path /path/to/model.pt
    Background: nohup python run_aladyn_predict.py --trained_model_path /path/to/model.pt > predict.log 2>&1 &
"""
...
def load_model_essentials(base_path):
    ...
    E = torch.load(base_path + 'E_enrollment_full.pt', weights_only=False)  # Enrollment data
...
def main():
    ...
    # Load trained model to get phi and psi
    print(f"Loading trained model from {args.trained_model_path}...")
    total_checkpoint = torch.load(args.trained_model_path, map_location='cpu', weights_only=False)
    phi_total = total_checkpoint['model_state_dict']['phi'].cpu().numpy()
    psi_total = total_checkpoint['model_state_dict']['psi'].cpu().numpy()

    print(f"Loaded phi shape: {phi_total.shape}")
    print(f"Loaded psi shape: {psi_total.shape}")
    ...
    for batch_idx, (start, stop) in enumerate(batches):
        ...
        try:
            ...
            # Reinitialize with initial_psi for consistent gamma initialization across batches
            print("Reinitializing gamma with initial_psi for consistency...")
            model.initialize_params(init_psi=torch.tensor(initial_psi, dtype=torch.float32))
            print("✓ Gamma reinitialized with initial_psi")
