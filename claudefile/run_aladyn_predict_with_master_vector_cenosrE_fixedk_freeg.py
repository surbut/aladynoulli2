#!/usr/bin/env python
"""
Prediction script for Aladyn model using enrollment data with fixed phi/psi and fixed kappa (gamma free)
USES MASTER CHECKPOINT FILES (pooled phi + initial_psi) + POOLED KAPPA

This script:
- Uses FIXED phi from pooled batches (master checkpoint) and initial_psi
- Uses FIXED kappa from pooled training batches
- LEARNS gamma per batch (free)
- Only estimates lambda and gamma per batch
...
Pooled kappa should be created using pool_kappa_and_gamma_from_batches.py:
- pooled_kappa_gamma.pt (pooled kappa)

Usage:
    python run_aladyn_predict_with_master_vector_cenosrE_fixedk_freeg.py \
        --trained_model_path /path/to/master.pt \
        --pooled_kappa_path /path/to/pooled_kappa_gamma.pt
"""
...
# Add AWS directory to path to use the fixed kappa model
sys.path.insert(0, str(Path(__file__).parent / 'aws_offsetmaster'))

from clust_huge_amp_fixedPhi_vectorized_fixed_kappa import *
import pandas as pd
...
def main():
    parser = argparse.ArgumentParser(description='Run Aladyn predictions with fixed phi/psi and fixed kappa (gamma free)')
    parser.add_argument('--trained_model_path', type=str, required=True,
                       help='Path to trained model with phi and psi')
    parser.add_argument('--pooled_kappa_path', type=str, required=True,
                       help='Path to pooled kappa file (from pool_kappa_and_gamma_from_batches.py)')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_fixedk_freeg_vectorized/',
    ...
    parser.add_argument('--profile', action='store_true',
                       help='Enable profiling (adds overhead, only use for debugging)')
    ...
    print(f"\n{'='*80}")
    print(f"Aladyn Prediction Script - Fixed Phi/Psi/Kappa Mode (Gamma Free)")
    print(f"{'='*80}")
    print(f"Trained model: {args.trained_model_path}")
    print(f"Pooled kappa: {args.pooled_kappa_path}")
    ...
    # Load initial_psi for consistent gamma initialization
    print("Loading initial_psi for consistent gamma initialization...")
    ...
    # Load pooled kappa
    print(f"Loading pooled kappa from {args.pooled_kappa_path}...")
    pooled_gk = torch.load(args.pooled_kappa_path, map_location='cpu', weights_only=False)
    
    if 'kappa' in pooled_gk:
        kappa_pooled = pooled_gk['kappa']
    else:
        raise ValueError(f"kappa not found in {args.pooled_kappa_path}")
    ...
    for batch_idx, (start, stop) in enumerate(batches):
        ...
        try:
            ...
            # Initialize model with FIXED phi, psi, and kappa (gamma is free)
            print(f"Initializing model with fixed phi/psi/kappa (gamma free)...")
            model = AladynSurvivalFixedPhiFixedKappa(
                N=Y_batch.shape[0],
                D=Y_batch.shape[1],
                T=Y_batch.shape[2],
                K=20,
            ...
            # Verify phi, psi, and kappa are fixed
            ...
            print(f"✓ gamma is free (will be learned per batch)")
            ...
            # Train model (lambda and gamma are being estimated, kappa is fixed)
            print(f"Training model (estimating lambda and gamma, kappa is fixed)...")
            ...
            with torch.no_grad():
                ...
                # Save model state (includes learned gamma)
                ...
                torch.save({
                    ...
                    'start_index': start,
                    'end_index': stop,
                    'learned_gamma': model.gamma.detach().cpu().numpy(),  # Save learned gamma per batch
                }, model_filename)
                print(f"✓ Saved model to {model_filename}")
                print(f"  Learned gamma shape: {model.gamma.shape}")
            ...
            batch_time_min = batch_time / 60.0
            print(f"✓ Batch {batch_idx+1}/{len(batches)} complete! (Time: {batch_time_min:.2f} minutes)")
    ...
    if pi_batches:
        ...
        batch_info = {
            ...
            'note': 'kappa fixed, gamma free (learned per batch)',
        }
    ...
    print(f"\n{'='*80}")
    print(f"DONE! Predictions use fixed kappa (gamma was learned per batch).")
    print(f"{'='*80}")
