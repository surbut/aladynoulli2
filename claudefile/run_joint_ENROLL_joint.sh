...
# nohup ./run_joint_ENROLL_joint.sh > joint_ENROLL_joint.log 2>&1 &
set -e  # Exit on error
...
for ((i=0; i<NUM_BATCHES; i++)); do
    ...
    python run_aladyn_batch_ENROLL_joint.py \
        --start_index $START \
        --end_index $END \
        --num_epochs $NUM_EPOCHS \
        --include_pcs $INCLUDE_PCS \
        2>&1 | tee "logs/joint_ENROLL_batch_${START}_${END}.log"

    if [ ${PIPESTATUS[0]} -eq 0 ]; then
        echo "✓ Batch $((i+1))/$NUM_BATCHES completed successfully at $(date)"
    else
        echo "✗ Batch $((i+1))/$NUM_BATCHES FAILED!"
        echo "Check log: logs/joint_ENROLL_batch_${START}_${END}.log"
        exit 1
    fi
