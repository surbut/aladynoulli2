#!/usr/bin/env python
"""
Nokappa v3 with gradient clipping: constant LR 0.1, 300 epochs, clip_grad_norm=5.0.
Same as train_nokappa_v3.py but with clipping for stability.

Usage:
    python claudefile/train_nokappa_v3_clip.py --start_index 0 --end_index 10000 --W 0.0001
"""
...
def fit_v3_clip(model, event_times, num_epochs=300, learning_rate=0.1, grad_clip=5.0):
    """Constant LR + gradient clipping."""
    param_groups = [
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            print(f"Epoch {epoch:4d}: Loss={cur_loss:.4f}, |gamma|={gamma_mag:.4f}, "
                  f"clip={grad_clip}, time={elapsed:.1f}min")

    return losses


def main():
    parser = argparse.ArgumentParser(description='Train Aladyn nokappa v3 with clipping')
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f'/Users/sarahurbut/Library/CloudStorage/Dropbox/nokappa_v3_clip_W{w_str}'

    torch.manual_seed(42)
    np.random.seed(42)

    print(f"\n{'='*60}")
    print(f"NOKAPPA v3 CLIP: samples {args.start_index} to {args.end_index}")
    print(f"Epochs: {args.num_epochs}, LR: {args.learning_rate} (constant), grad_clip: {args.grad_clip}")
    ...
    # Train
    print(f"\nTraining (constant LR={args.learning_rate}, grad_clip={args.grad_clip}, "
          f"{args.num_epochs} epochs)...")
    t0 = time.time()
    losses = fit_v3_clip(model, E_batch,
    ...
    output_path = output_dir / f'enrollment_model_REPARAM_NOKAPPA_CLIP_W{args.W}_batch_{args.start_index}_{args.end_index}.pt'
