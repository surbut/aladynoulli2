...
#master_for_fitting_pooled_correctedE.pt with zero genetic effects (0G)
nohup python run_aladyn_predict_with_master_vector_cenosrE_withzerog.py \
    ...
    --output_dir /Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_correctedE_vectorized_0G/ \
    --max_batches 5 \
    > predict_correctedE_vectorized_0G.log 2>&1 &
"""
...
from clust_huge_amp_fixedPhi_vectorized_withLR import *
import pandas as pd
...
def main():
    ...
    for batch_idx, (start, stop) in enumerate(batches):
        ...
        try:
            ...
            # Get demographics - EXCLUDE genetic features, only use sex and PCs
            pce_df_subset = fh_processed.iloc[start:stop].reset_index(drop=True)
            sex = pce_df_subset['sex'].values.reshape(-1, 1)  # Reshape to column vector
            
            # Build G matrix WITHOUT genetic features - only sex and optionally PCs
            if args.include_pcs:
                ...
                G_no_genetics = np.column_stack([sex, pcs])
                print(f"Using sex + {len(pc_columns)} PCs (NO genetic features)")
            else:
                G_no_genetics = sex
                print(f"Using sex only (NO genetic features)")

            print(f"Data shapes: Y={Y_batch.shape}, E={E_batch.shape}, G={G_no_genetics.shape}")
            print(f"Original G_batch shape (genetics): {G_batch.shape} - EXCLUDED")

            # Initialize model with FIXED phi and psi
            # Use normal genetic_scale since we've excluded genetics from G
            genetic_scale = 1.0  # Normal scale, but genetics aren't in G anyway
            print(f"Genetic scale: {genetic_scale} (genetics excluded from G matrix)")
            ...
            with torch.no_grad():
                ...
                torch.save({
                    ...
                    'start_index': start,
                    'end_index': stop,
                    'genetic_scale': model.genetic_scale,
                }, model_filename)
            ...
            del pi, model, Y_batch, E_batch, G_batch, G_no_genetics, pce_df_subset
            if torch.cuda.is_available():
