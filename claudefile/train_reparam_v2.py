#!/usr/bin/env python
"""
Improved reparam training: cosine annealing LR, gradient clipping, more epochs.
Does NOT modify the production model class — overrides fit() externally.

Usage:
    python train_reparam_v2.py --start_index 0 --end_index 10000 --num_epochs 500
"""
...
def fit_v2(model, event_times, num_epochs=500, learning_rate=0.1,
    ...
    Does NOT modify the model class — uses the model's compute_loss/forward externally.
    """
    param_groups = [
    ...
    if isinstance(model.kappa, nn.Parameter):
        param_groups.append({'params': [model.kappa], 'lr': learning_rate, 'label': 'kappa'})
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            kappa_val = model.kappa.item() if isinstance(model.kappa, nn.Parameter) else float(model.kappa)
            gamma_mag = model.gamma.abs().mean().item()
            print(f"Epoch {epoch:4d}: Loss={cur_loss:.4f}, LR={lr_now:.1e}, "
                  f"kappa={kappa_val:.3f}, |gamma|={gamma_mag:.4f}, "
...
def main():
    parser = argparse.ArgumentParser(description='Train Aladyn REPARAM v2 (improved optimization)')
    ...
    print(f"\n{'='*60}")
    print(f"REPARAM v2 Training: samples {args.start_index} to {args.end_index}")
    ...
    kappa_val = model.kappa.item() if isinstance(model.kappa, nn.Parameter) else float(model.kappa)
    gamma_mag = model.gamma.abs().mean().item()
    print(f"\nFinal params: kappa={kappa_val:.4f}, mean|gamma|={gamma_mag:.4f}")
