...
LOOP VERSION (non-vectorized) - for comparison with vectorized version
...
Usage:
    Local:  python run_aladyn_predict_with_master_loop_cenosrE.py --trained_model_path /path/to/master.pt
    Background: cd /Users/sarahurbut/aladynoulli2/claudefile

# Fixed phi from pooled enrollment data (10 batches)
nohup python run_aladyn_predict_with_master_loop_cenosrE.py \
    ...
    --output_dir /Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_ENROLLMENT_pooled_loop/ \
    --max_batches 10 \
    > predict_enrollment_pooled_loop.log 2>&1 &
...
nohup python run_aladyn_predict_with_master_loop_cenosrE.py \
    ...
    --output_dir /Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_RETROSPECTIVE_pooled_loop/ \
    --max_batches 10 \
    > predict_retrospective_pooled_loop.log 2>&1 &
...
nohup python run_aladyn_predict_with_master_loop_cenosrE.py \
    ...
    --output_dir /Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_correctedE_loop/ \
    --max_batches 40 \
    > predict_correctedE_loop.log 2>&1 &
"""
import numpy as np
import torch
import warnings
...
# Add parent directory to path to use the loop (non-vectorized) model
...
def main():
    parser = argparse.ArgumentParser(description='Run Aladyn predictions with fixed phi/psi (LOOP VERSION)')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_withpcs_batchrun_loop/',
    ...
    print(f"\n{'='*80}")
    print(f"Aladyn Prediction Script - Fixed Phi/Psi Mode (LOOP VERSION)")
    print(f"{'='*80}")
