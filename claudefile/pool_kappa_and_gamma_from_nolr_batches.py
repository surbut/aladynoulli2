#!/usr/bin/env python
"""
Pool kappa and gamma values from _nolr training batches (no regularization).

This script:
- Loads kappa and gamma from all _nolr batch files
- Computes mean kappa (pooled across batches)
- Computes mean gamma (pooled across batches) - these will be LARGER than regularized
- Saves the pooled values for use in fixed-kappa, fixed-gamma models

Usage:
    python pool_kappa_and_gamma_from_nolr_batches.py
"""
...
def pool_kappa_and_gamma_from_batches(pattern, max_batches=None):
            ...
            if gamma is not None:
                ...
                if not np.allclose(gamma, 0):
                    ...
                    if len(all_gammas) <= 3:  # Print first 3
                        ...
                        print(f"  Loaded kappa={kappa_str}, gamma shape={gamma.shape} from {Path(file_path).name}")
                else:
    ...
    print(f"\n{'='*80}")
    print(f"GAMMA POOLING RESULTS (NO REGULARIZATION - EXPECTED TO BE LARGER)")
    print(f"{'='*80}")
    ...
    print(f"  ⚠️  NOTE: These gamma values are from UNREGULARIZED training (no lambda_reg)")
    print(f"      They should be LARGER than regularized gamma values")
    
    return kappa_pooled, gamma_pooled, kappa_array, len(all_gammas)

def main():
    parser = argparse.ArgumentParser(description='Pool kappa and gamma from _nolr training batch files (no regularization)')
    parser.add_argument('--batch_pattern', type=str,
                        ...
                        help='Pattern for _nolr training batch files to pool from (e.g., enrollment_model_VECTORIZED_W0.0001_nolr_batch_*_*.pt)')
    ...
    print("="*80)
    print("POOLING KAPPA AND GAMMA FROM _NOLR TRAINING BATCHES (NO REGULARIZATION)")
    print("="*80)
    ...
    try:
        ...
        # Save kappa (with _nolr suffix)
        kappa_path = output_dir / 'pooled_kappa_nolr.pt'
        torch.save({'kappa': kappa_pooled, 'kappa_array': kappa_array, 'n_batches': n_batches}, kappa_path)
        print(f"\n✓ Saved pooled kappa to: {kappa_path}")
        
        # Save gamma (with _nolr suffix)
        gamma_path = output_dir / 'pooled_gamma_nolr.pt'
        torch.save({'gamma': gamma_pooled, 'n_batches': n_batches}, gamma_path)
        print(f"✓ Saved pooled gamma to: {gamma_path}")
        
        # Save both together for convenience (with _nolr suffix)
        combined_path = output_dir / 'pooled_kappa_gamma_nolr.pt'
        torch.save({
            'kappa': kappa_pooled,
            'gamma': gamma_pooled,
        ...
        print(f"  These are from UNREGULARIZED training (no lambda_reg)")
        print(f"  Gamma values should be LARGER than regularized version")
        print(f"  Both should be fixed during prediction (not learned per batch)")
        print(f"\n  To use in fixed-gamma/kappa predictions:")
        print(f"    --pooled_gamma_kappa_path {combined_path}")
