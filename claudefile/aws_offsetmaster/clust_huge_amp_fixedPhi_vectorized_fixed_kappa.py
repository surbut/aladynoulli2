...
class AladynSurvivalFixedPhiFixedKappa(nn.Module):
    """Version with phi and kappa fixed - only trains lambda and gamma"""
    def __init__(self, N, D, T, K, P, G, Y, R, W, prevalence_t, init_sd_scaler, genetic_scale,
                 pretrained_phi, pretrained_psi, pretrained_kappa, signature_references=None, 
    ...
    def initialize_params(self, init_psi=None, init_gamma=None, **kwargs):
        ...
        # Only make lambda and gamma trainable parameters (kappa is fixed)
        ...
        print("Initialization complete! (kappa is fixed)")
    ...
    def fit(self, event_times, num_epochs=100, learning_rate=0.01, lambda_reg=0.01):
        """Modified fit method that only updates lambda and gamma (kappa is fixed)"""
        
        optimizer = optim.Adam([
            {'params': [self.lambda_], 'lr': learning_rate},      # e.g. 1e-2
            {'params': [self.gamma], 'lr': learning_rate},       # Same rate as lambda
            # kappa is NOT in optimizer - it's a fixed buffer
        ])
