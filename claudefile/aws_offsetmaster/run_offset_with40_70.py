...
def main():
    ...
    # Age offset 0 = age 40, age offset 30 = age 70
    ...
    for age_offset in range(0, args.max_age_offset + 1):
        ...
        for test_idx in test_patients:
            if test_idx < len(pce_df_subset):
                ...
                # Current age is fixed (40 + age_offset), not enrollment_age + age_offset
                ...
                print(f"  Patient {test_idx}: enrollment={enrollment_age:.0f}, fixed_age={current_age_check:.0f}, "
        ...
        with torch.no_grad():
            ...
            pi_filename = os.path.join(args.output_dir, 
                f'pi_fixedphi_age_{current_age}_offset_{age_offset}_batch_{start_index}_{end_index}.pt')
            torch.save(pi, pi_filename)
            print(f"Saved predictions to {pi_filename}")
        
        # Save model checkpoint
        model_filename = os.path.join(args.output_dir,
            f'model_fixedphi_age_{current_age}_offset_{age_offset}_batch_{start_index}_{end_index}.pt')
        torch.save({
