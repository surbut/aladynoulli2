...
# Add scripts directory to path to import clust_huge_amp
# Try multiple possible locations (for local vs AWS)
script_dir = Path(__file__).parent
possible_paths = [
    script_dir,  # Same directory as script (if clust_huge_amp.py is in scripts/)
    script_dir.parent / 'scripts',  # Parent/scripts (if script is in ~/aladyn_project/scripts/)
    Path.home() / 'aladyn_project' / 'scripts',  # ~/aladyn_project/scripts/
    Path.home() / 'pyScripts_forPublish',  # Fallback: old location
    script_dir.parent / 'pyScripts_forPublish',  # Fallback: old location
]
for path in possible_paths:
    if (path / 'clust_huge_amp.py').exists() or (path / 'clust_huge_amp').exists():
        sys.path.insert(0, str(path))
        break
else:
    # If none found, print error
    print(f"ERROR: Could not find clust_huge_amp.py. Checked:")
    for path in possible_paths:
        print(f"  - {path}")
    sys.exit(1)
...
def main():
    ...
    # Path configuration - defaults to AWS paths, can be overridden
    parser.add_argument('--data_dir', type=str,
                       default='/data',
                       help='Directory containing input data (default: /data for AWS)')
    parser.add_argument('--output_dir', type=str,
                       default='/results',
                       help='Output directory for saved models (default: /results for AWS)')
    parser.add_argument('--covariates_path', type=str,
                       default='/data/baselinagefamh_withpcs.csv',
                       help='Path to covariates CSV file (default: /data/baselinagefamh_withpcs.csv)')
    ...
    # Load data
    Y, E, G, essentials = load_model_essentials(data_dir)
