#!/usr/bin/env python
"""
Nokappa v3 training (AWS version): constant LR, no cosine, no clipping, 300 epochs.
Grid search winner recipe applied to no-kappa reparam.

Usage:
    python train_nokappa_v3_aws.py --start_index 0 --end_index 10000 --W 0.0001
"""
...
# Import reparam model from same directory
...
def main():
    parser = argparse.ArgumentParser(description='Train Aladyn nokappa v3 (constant LR) - AWS')
    ...
    if args.output_dir is None:
        args.output_dir = '/home/ubuntu/aladyn_project/output/nokappa_v3_production/'

    torch.manual_seed(42)
    np.random.seed(42)

    print(f"\n{'='*60}")
    print(f"NOKAPPA v3 (AWS): samples {args.start_index} to {args.end_index}")
    ...
    # Load data — AWS uses E_enrollment_full.pt
