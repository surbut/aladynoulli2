#!/usr/bin/env python
"""
PRODUCTION RUN (AWS): Nokappa v3 (constant LR) on ALL 40 batches (0-39, 400K patients).
...
Usage:
    python run_nokappa_v3_production_aws.py                # All 40 batches
    python run_nokappa_v3_production_aws.py --n_batches 2  # Quick test (2 batches)
    python run_nokappa_v3_production_aws.py --resume_from 15  # Resume from batch 15
"""
...
def run_batch(start, end, output_dir, script, data_dir, cov_path):
...
def main():
    parser = argparse.ArgumentParser(
        description='PRODUCTION (AWS): Run nokappa v3 (constant LR) on all 40 batches')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/home/ubuntu/aladyn_project/output/nokappa_v3_production/',
                        help='Output directory for checkpoints')
    parser.add_argument('--data_dir', type=str,
                        default='/home/ubuntu/aladyn_project/data_for_running/',
                        help='Data directory')
    parser.add_argument('--covariates_path', type=str,
                        default='/home/ubuntu/aladyn_project/data_for_running/baselinagefamh_withpcs.csv',
                        help='Covariates CSV path')
    args = parser.parse_args()

    script = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'train_nokappa_v3_aws.py')
    last_batch = min(args.resume_from + args.n_batches, TOTAL_BATCHES)

    print(f"\n{'='*70}")
    print(f"PRODUCTION (AWS): Nokappa v3 (constant LR=0.1, W=1e-4, kappa=1, 300 epochs)")
    ...
    for i in range(args.resume_from, last_batch):
        ...
        elapsed, ok = run_batch(start, end, args.output_dir, script,
                                args.data_dir, args.covariates_path)
        if ok:
            completed += 1
