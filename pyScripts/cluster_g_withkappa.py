...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest_withkappa(nn.Module):
    def __init__(self, N, D, T, K, P, G, Y, prevalence_t,init_var_scaler, genetic_scale,signature_references=None, healthy_reference=None, disease_names=None,flat_lambda=False):
        ...
        self.kappa = nn.Parameter(torch.ones(1))  # Initialize to 1.0
   ...
   # Initialize psi for disease clusters
    ...
    def compute_loss(self, event_times):
        ...
        # Add logging to see components affected by kappa
        print(f"Mean pi: {pi.mean().item():.3e}")
    ...
    def fit(self, event_times, num_epochs=1000, learning_rate=1e-4, lambda_reg=1e-2,
        ...
        """
            ...
            # Create optimizer with separate learning rate for kappa
        initial_lr = learning_rate / 100 
        kappa_lr = learning_rate * 1000  # Much larger learning rate for kappa (100x instead of 10x)
        ...
        optimizer = optim.Adam([
            ...
            {'params': [self.kappa], 'lr': kappa_lr}  # Much larger learning rate
        ])
        ...
        for epoch in range(num_epochs):
            if epoch < warmup_epochs:
                ...
                # Apply warmup to all parameter groups except kappa
                for param_group in optimizer.param_groups[:-1]:  # Skip last group (kappa)
            ...
            if epoch < 10 or epoch % 10 == 0:
                ...
                print(f"Epoch {epoch}, Kappa: {self.kappa.item():.3f}")
