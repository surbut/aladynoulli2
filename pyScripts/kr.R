...
gen=read.csv("~/aladynoulli2/pyScripts/all_patient_genetics.csv")
prs_names=read.csv("prs_names.csv")
all_patient_diseases=data.frame(read.csv("all_patient_diseases.csv",fill = T))
Y=data.frame(read.csv("Y_summed_400k.csv"))
sig_refs=read.csv("reference_thetas.csv",header = T)
...
p1=ggplot(m, aes(x=Time, y=Value, fill=as.factor(Sig))) +
    ...
    theme_classic() +
    labs(fill="Signature", y=paste0("Theta Deviation from Reference ,",name), x="Time")
  
  ggsave(plot = p1,filename =paste0("stackedtraj",name,".pdf"),dpi=300)
...
library(tidyr)
  ...
  write.csv(prs_means_by_cluster, paste0("prs_means_by_cluster",name,".csv"))
  ...
  absmin=min(m$value)
  absmax=max(m$value)
  p2=ggplot(m,aes(x=cluster,y=variable,fill=value))+geom_tile()+scale_fill_gradient2(
    low = "blue", high = "red", mid = "white", 
    midpoint = 0,limits=c(absmin-0.01,absmax+0.01))+labs(x="Grouping",y="PRS",fill="Mean Value")
  print(p2)
  ...
  for (c in 1:n_clusters) {
    ...
    for (r in 1:R) {
      ...
      prs_long <- rbind(prs_long, data.frame(
        Cluster = c,
        PRS = prs_names[r,1],
        Mean = mean_in,
        D = d
  ...
  p <- ggplot(prs_long, aes(x = as.factor(Cluster), y = PRS, fill = Mean)) +
    geom_tile(color = "white") +
    scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0) +
    geom_text(aes(label = sprintf("%.2f", Mean)), size = 3, color = "black") +
    labs(x = "Cluster", y = "PRS", fill = "Mean PRS", title = paste0("PRS Means by Cluster ",name)) +
  ...
  ggsave(plot = p,filename =paste0("stackedgenetics",name,".pdf"),dpi=300)
...
library(ggsci);
ggplot(m,aes(x=cluster,y=variable,fill=value))+geom_tile()+scale_fill_gradient2(
low = "blue", high = "red", mid = "white", 
midpoint = 0)
return(clust)
}
