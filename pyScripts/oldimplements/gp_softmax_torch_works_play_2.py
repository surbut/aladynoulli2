...
class AladynSurvivalModel(nn.Module):
    ...
    def update_kernels(self):
        ...
        # Target condition number (A condition number is a measure of how sensitive a matrix is to numerical operations. It's calculated as the ratio of the largest to smallest singular values of a matrix:)
        max_condition = 1e4
        ...
        for k in range(self.K):
            ...
            # Compute base kernel (i.e., RBF kereenl)
    ...
    def compute_loss(self, event_times):
        ...
        loss_censored = -torch.sum(torch.log(1 - pi) * mask_before_event) # Contributes to loss for all times before event, Represents survival until event time
        loss_event = -torch.sum(torch.log(pi) * mask_at_event * self.Y) # Log probability of event occurring
        loss_no_event = -torch.sum(torch.log(1 - pi) * mask_at_event * (1 - self.Y)) # Log probability of no event
    ...
    def fit(self, event_times, num_epochs=1000, learning_rate=5e-4):
        optimizer = optim.Adam([
            {'params': [self.lambda_, self.phi], 'lr': learning_rate},
            {'params': [self.length_scales, self.log_amplitudes], 'lr': learning_rate * 0.1},
            {'params': [self.gamma], 'lr': learning_rate * 0.01}  # Slower updates for genetic effects
        ])
        ...
        for epoch in range(num_epochs):
            optimizer.zero_grad() ## zeroes out graduation
            ...
            loss = self.compute_loss(event_times) ## computing loss, uses forward implicitly to calucate pi, theta, and eta
            # Implicitly calls forward() to get pi, theta, phi
            # pi = P(event at time t)
            # theta = individual topic weights
            # phi = disease patterns per topic

            loss.backward() ### calculate gradients of each parameter (i.e., dL/dPi *dPi/dParam)
            # Calculates gradients for all parameters using chain rule:
            # dL/dλ = dL/dπ * dπ/dθ * dθ/dλ
            # dL/dφ = dL/dπ * dπ/dφ
            # dL/dγ = dL/dπ * dπ/dθ * dθ/dλ * dλ/dγ
            ...
            optimizer.step() ## adjusts paramters values
