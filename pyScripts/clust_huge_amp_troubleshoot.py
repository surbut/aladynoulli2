...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest(nn.Module):
    ...
    def compute_loss(self, event_times):
        """Modified loss function with debugging prints"""
        ...
        # Debug prints for individual loss components
        print("\nLoss Components:")
        print(f"Loss censored: {loss_censored.item()}, is NaN: {torch.isnan(loss_censored).item()}")
        print(f"Loss event: {loss_event.item()}, is NaN: {torch.isnan(loss_event).item()}")
        print(f"Loss no event: {loss_no_event.item()}, is NaN: {torch.isnan(loss_no_event).item()}")
        
        total_data_loss = (loss_censored + loss_event + loss_no_event) / (self.N)
        print(f"Total data loss: {total_data_loss.item()}, is NaN: {torch.isnan(total_data_loss).item()}")

        if self.gpweight > 0:
            gp_loss = self.compute_gp_prior_loss()
            print(f"GP loss: {gp_loss.item()}, is NaN: {torch.isnan(gp_loss).item()}")
        else:
            gp_loss = 0.0
    ...
    def compute_gp_prior_loss(self):
        """GP prior loss with debugging"""
        gp_loss_lambda = 0.0
        gp_loss_phi = 0.0
        
        try:
            L_lambda = torch.linalg.cholesky(self.K_lambda)
            print("Lambda Cholesky decomposition successful")
        except Exception as e:
            print(f"Lambda Cholesky failed: {e}")
            return float('nan')
        
        try:
            L_phi = torch.linalg.cholesky(self.K_phi)
            print("Phi Cholesky decomposition successful")
        except Exception as e:
            print(f"Phi Cholesky failed: {e}")
            return float('nan')
        
        # Debug prints for kernel matrices
        print(f"\nKernel Statistics:")
        print(f"K_lambda condition number: {torch.linalg.cond(self.K_lambda).item()}")
        print(f"K_phi condition number: {torch.linalg.cond(self.K_phi).item()}")
        
        for k in range(self.K_total):
            lambda_k = self.lambda_[:, k, :]
            
            if k == self.K and self.healthy_ref is not None:
            ...
            for i in range(self.N):
                ...
                # Check for NaNs in GP computation
                if torch.isnan(gp_loss_lambda):
                    print(f"NaN detected in lambda GP loss at k={k}, i={i}")
                    print(f"dev_i stats: mean={dev_i.mean().item()}, std={dev_i.std().item()}")
                    break
        ...
        for k in range(self.K_total):
            ...
            for d in range(self.D):
                ...
                # Check for NaNs in GP computation
                if torch.isnan(gp_loss_phi):
                    print(f"NaN detected in phi GP loss at k={k}, d={d}")
                    print(f"dev_d stats: mean={dev_d.mean().item()}, std={dev_d.std().item()}")
                    break
        
        final_gp_loss = gp_loss_lambda / self.N + gp_loss_phi / self.D
        print(f"\nFinal GP loss components:")
        print(f"Lambda GP loss: {(gp_loss_lambda / self.N).item()}")
        print(f"Phi GP loss: {(gp_loss_phi / self.D).item()}")
        
        return final_gp_loss

        
    def fit(self, event_times, num_epochs=100, learning_rate=0.01, lambda_reg=0.01):
        """Modified fit method with detailed debugging"""
        param_groups = [
        ...
        for epoch in range(num_epochs):
            ...
            # Pre-loss parameter statistics
            print("\nPre-loss Parameter Stats:")
            print(f"Lambda: mean={self.lambda_.mean().item():.4f}, std={self.lambda_.std().item():.4f}")
            print(f"Phi: mean={self.phi.mean().item():.4f}, std={self.phi.std().item():.4f}")
            ...
            # Gradient statistics
            print("\nGradient Statistics:")
            print(f"Lambda grad: mean={self.lambda_.grad.mean().item():.4f}, std={self.lambda_.grad.std().item():.4f}")
            print(f"Phi grad: mean={self.phi.grad.mean().item():.4f}, std={self.phi.grad.std().item():.4f}")
            print(f"Lambda grad has NaN: {torch.isnan(self.lambda_.grad).any().item()}")
            print(f"Phi grad has NaN: {torch.isnan(self.phi.grad).any().item()}")
            ...
            # Post-update parameter statistics
