...
all_thetas=readRDS("big_stuff/all_patient_thetas_alltime.rds")
...
num_diag_after=sapply(seq(1:nrow(E_full)),function(x){
  sum(E_enroll[x,]>pce_data[x,age-30]&E_enroll[x,]!=51)
}
)

table(num_diag)  # See distribution
candidate_patients <- which(num_diag >= 5 & num_diag <= 10)
#set.seed(1)
chosen_patient <- sample(candidate_patients, 1)
...
ggsave(plot = a,filename = "longpattraj.pdf",dpi = 300,width = 25,height = 10)