...
class AladynSurvivalFixedKernelsAvgLoss(nn.Module):
    ...
    def initialize_params(self):
        """Initialize parameters using SVD and prevalence-scaled GP priors"""
        # Compute average disease occurrence matrix (N x D)
        Y_avg = torch.mean(self.Y, dim=2)
        
        # Center and scale Y by prevalence
        disease_means = torch.mean(Y_avg, dim=0)  # D
        prevalence_scale = torch.mean(self.prevalence_t, dim=1)  # D
        Y_scaled = Y_avg / (prevalence_scale + 1e-8)[None, :]
        
        # Perform SVD on scaled data
        U, S, Vh = torch.linalg.svd(Y_scaled, full_matrices=False)
        ...
        # Initialize lambda incorporating prevalence structure
        lambda_means = self.G @ self.gamma
        self.lambda_ = nn.Parameter(torch.zeros((self.N, self.K, self.T)))
        
        # Initialize phi using both SVD structure and prevalence
        phi_init = torch.zeros((self.K, self.D, self.T))
        Vh_scaled = Vh[:self.K, :] * torch.sqrt(S[:self.K])[:, None]
        
        # For each time point, scale the SVD components by prevalence
        for t in range(self.T):
            phi_init[:, :, t] = Vh_scaled * torch.log(self.prevalence_t[:, t] + 1e-8)
        ...
        # Sample initial values from GPs with informed means
        for k in range(self.K):
            L_lambda = torch.linalg.cholesky(self.K_lambda[k])
            L_phi = torch.linalg.cholesky(self.K_phi[k])
            
            # Sample lambda with SVD-based mean
            for i in range(self.N):
                mean = lambda_means[i, k]
                eps = L_lambda @ torch.randn(self.T)
                self.lambda_.data[i, k, :] = mean + eps
            
            # Sample phi with combined SVD and prevalence-based mean
            for d in range(self.D):
                mean = phi_init[k, d, :] + self.logit_prev_t[d, :]
...
def plot_topic_diagnostics(self, disease_names=None, top_n=10):
    """
    Plot diagnostic visualizations for topics:
    1. Top diseases per topic based on phi values
    2. Topic trajectories over time
    3. Disease clustering by topic loadings
    
    Parameters:
    disease_names: list of disease names (if None, will use indices)
    top_n: number of top diseases to show per topic
    """
    if disease_names is None:
        disease_names = [f"Disease_{i}" for i in range(self.D)]
    
    # Convert phi to probabilities
    phi_prob = torch.sigmoid(self.phi.detach())  # K x D x T
    
    # Calculate average influence of each disease in each topic
    disease_importance = phi_prob.mean(dim=2)  # K x D
    
    plt.figure(figsize=(15, 5 * self.K))
    
    for k in range(self.K):
        # Plot top diseases for this topic
        plt.subplot(self.K, 2, 2*k + 1)
        
        # Get top diseases for this topic
        top_diseases_idx = torch.argsort(disease_importance[k], descending=True)[:top_n]
        top_values = disease_importance[k][top_diseases_idx]
        top_names = [disease_names[i] for i in top_diseases_idx]
        
        # Create barplot
        plt.barh(range(top_n), top_values)
        plt.yticks(range(top_n), top_names)
        plt.title(f'Topic {k+1}: Top {top_n} Diseases')
        plt.xlabel('Average Disease Probability')
        
        # Plot topic trajectories over time
        plt.subplot(self.K, 2, 2*k + 2)
        for i in top_diseases_idx[:5]:  # Plot top 5 diseases' trajectories
            plt.plot(phi_prob[k, i].numpy(), 
                    label=disease_names[i][:30])  # Truncate long names
        plt.title(f'Topic {k+1}: Disease Trajectories Over Time')
        plt.xlabel('Time')
        plt.ylabel('Probability')
    ...
    # Additional visualization: Disease clustering by topic loadings
    plt.figure(figsize=(12, 8))
    
    # Use t-SNE to visualize disease relationships based on topic loadings
    from sklearn.manifold import TSNE
    
    # Prepare disease-topic matrix
    disease_topic_matrix = disease_importance.T.numpy()  # D x K
    
    # Apply t-SNE
    tsne = TSNE(n_components=2, random_state=42)
    disease_embedding = tsne.fit_transform(disease_topic_matrix)
    
    # Plot disease clusters
    plt.scatter(disease_embedding[:, 0], disease_embedding[:, 1], alpha=0.5)
    
    # Add labels for top diseases in each topic
    for k in range(self.K):
        top_diseases_idx = torch.argsort(disease_importance[k], descending=True)[:5]
        for idx in top_diseases_idx:
            plt.annotate(disease_names[idx][:30], 
                        (disease_embedding[idx, 0], disease_embedding[idx, 1]))
    
    plt.title('Disease Clustering by Topic Loadings')
    plt.xlabel('t-SNE 1')
    plt.ylabel('t-SNE 2')
    plt.show()
    ...
    for k in range(self.K):
        ...
        # Calculate average pairwise correlation between top diseases
        top_disease_trajectories = phi_prob[k, top_diseases_idx].numpy()
        corr_matrix = np.corrcoef(top_disease_trajectories)
        avg_correlation = (corr_matrix.sum() - np.trace(corr_matrix)) / (corr_matrix.size - corr_matrix.shape[0])
        
        print(f"Average correlation between top diseases: {avg_correlation:.3f}")
        
        # Print top disease probabilities
        print("\nTop disease probabilities:")
        for idx, prob in zip(top_diseases_idx, disease_importance[k][top_diseases_idx]):
            print(f"{disease_names[idx][:50]}: {prob:.3f}")