...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest(nn.Module):
    ...
    def fit(self, event_times, num_epochs=1000, learning_rate=1e-4, lambda_reg=1e-2,
        ...
        for epoch in range(num_epochs):
            ...
            old_psi = self.psi.detach().clone()
            old_phi = self.phi.detach().clone()
            old_gamma = self.gamma.detach().clone()
            old_lambda=self.lambda_.detach().clone()
            optimizer.step()
            prev_loss = loss_val
            psi_change = (self.psi - old_psi).abs().mean().item()
            print(f"Average psi change: {psi_change:.4e}")
            phi_change = (self.phi - old_phi).abs().mean().item()
            print(f"Average phii change: {phi_change:.4e}")
            gamma_change = (self.gamma - old_gamma).abs().mean().item()
            print(f"Average gamma change: {gamma_change:.4e}")
