...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest_difflambda(nn.Module):
    ...
    def initialize_params(self, psi_config=None, true_psi=None, **kwargs):
        ...
        for k in range(self.K):
            ...
            for i in range(self.N):
                if true_psi is not None:
                    person_events = self.Y[i, strong_diseases > 0]
                    base_pattern = self.logit_prev_t[strong_diseases > 0].mean(dim=0)
                else:
                    person_events = self.Y[i, cluster_diseases]
                    base_pattern = self.logit_prev_t[cluster_diseases].mean(dim=0)
                    
                temporal_pattern = base_pattern.clone()
                
                # Create time points for Gaussian
                time_points = torch.arange(self.T, dtype=torch.float32)
                
                # Check for disease onsets
                for d in range(person_events.shape[0]):
                    onset_times = torch.where(person_events[d] == 1)[0]
                    if len(onset_times) > 0:
                        # For each onset, create a smooth Gaussian boost
                        for t in onset_times:
                            gaussian_boost = 0.5 * torch.exp(-((time_points - t)**2) / (2 * 5**2))
                            temporal_pattern += gaussian_boost
                
                # Add genetics
                genetic_score = (self.G @ gamma_init[:, k])[i]
                combined = temporal_pattern + genetic_score
                
                # Add small GP noise
                L_k = torch.linalg.cholesky(self.K_lambda[k])
                eps = 0.1 * (L_k @ torch.randn(self.T))
                lambda_init[i, k, :] = combined + eps
