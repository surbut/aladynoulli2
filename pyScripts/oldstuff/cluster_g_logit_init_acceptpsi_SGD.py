...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest(nn.Module):
    ...
    def fit(self, event_times, num_epochs=1000, batch_size=32, learning_rate=1e-4, lambda_reg=1e-2,
        ...
        event_times: array-like
            Array of event times for each individual and disease
        batch_size: int
        ...
        convergence_threshold: float
            Threshold for early stopping based on loss change
        patience: int
        ...
        for epoch in range(num_epochs):
            ...
            for b in range(num_batches):
                ...
                end_idx = min((b + 1) * batch_size, self.N)
                batch_indices = shuffled_indices[start_idx:end_idx]
                
                # Select batch data
                batch_Y = self.Y[batch_indices]
                batch_G = self.G[batch_indices]
                batch_event_times = event_times_tensor[batch_indices]
                
                # Compute loss for batch
                loss = self.compute_batch_loss(batch_indices, batch_event_times)
                loss_val = loss.item()
                epoch_loss += loss_val
            ...
            # Check convergence
            loss_change = abs(prev_loss - avg_epoch_loss)
    ...
    def compute_batch_loss(self, batch_indices, batch_event_times):
        """
        ...
        pi = self.forward()[batch_indices]
        
        # Compute loss components for batch
        epsilon = 1e-8
        pi = torch.clamp(pi, epsilon, 1 - epsilon)
        
        # Create masks for batch
        event_times_expanded = batch_event_times.unsqueeze(-1)
        time_grid = torch.arange(self.T).unsqueeze(0).unsqueeze(0)
        ...
        # Compute loss components
        loss_censored = -torch.sum(torch.log(1 - pi) * mask_before_event)
        loss_event = -torch.sum(torch.log(pi) * mask_at_event * self.Y[batch_indices])
        loss_no_event = -torch.sum(torch.log(1 - pi) * mask_at_event * (1 - self.Y[batch_indices]))
        
        # Normalize by batch size
        total_data_loss = (loss_censored + loss_event + loss_no_event) / batch_size
        
        # Add GP prior loss (scaled by batch size ratio)
        gp_loss = self.compute_gp_prior_loss() * (batch_size / self.N)
        
        return total_data_loss + gp_loss
