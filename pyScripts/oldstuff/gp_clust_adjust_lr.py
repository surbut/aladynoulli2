...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest_alr(nn.Module):
    ...
    def fit(self, event_times, num_epochs=1000, initial_lr=1e-3, lambda_reg=1e-2,
            convergence_threshold=1e-3, patience=10):
        """
        Fit model with learning rate schedulers for different parameter groups
        """
        # Create parameter groups with different learning rates
        param_groups = [
            {'params': [self.lambda_], 'lr': initial_lr, 'name': 'lambda'},
            {'params': [self.phi], 'lr': initial_lr, 'name': 'phi'},
            {'params': [self.psi], 'lr': initial_lr, 'name': 'psi'},
            {'params': [self.gamma], 'lr': initial_lr, 'weight_decay': lambda_reg, 'name': 'gamma'}
        ]
        optimizer = optim.Adam(param_groups)

        # Create schedulers for each parameter group
        schedulers = {
            'lambda': torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode='min', factor=0.5, patience=5, verbose=True
            ),
            'phi': torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode='min', factor=0.5, patience=5, verbose=True
            ),
            'psi': torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode='min', factor=0.5, patience=5, verbose=True
            ),
            'gamma': torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode='min', factor=0.5, patience=5, verbose=True
            )
        }
        ...
        history = {
            'loss': [],
            'learning_rates': {name: [] for name in ['lambda', 'phi', 'psi', 'gamma']},
            'gradients': {name: [] for name in ['lambda', 'phi', 'psi', 'gamma']}
        }
        ...
        # Create a mapping from name to param_group index
        param_group_indices = {group['name']: idx for idx, group in enumerate(optimizer.param_groups)}
        ...
        for epoch in range(num_epochs):
            ...
            # Track gradients and learning rates
            for param_group in optimizer.param_groups:
                name = param_group['name']
                grad = param_group['params'][0].grad
                if grad is not None:
                    history['gradients'][name].append(grad.abs().max().item())
                history['learning_rates'][name].append(param_group['lr'])

            if epoch % 10 == 0:
                print(f"\nEpoch {epoch}")
                print(f"Loss: {loss_val:.4f}")
                for name in ['lambda', 'phi', 'psi', 'gamma']:
                    idx = param_group_indices[name]
                    print(f"{name} - LR: {optimizer.param_groups[idx]['lr']:.2e}, "
                          f"Grad: {history['gradients'][name][-1]:.2e}")
