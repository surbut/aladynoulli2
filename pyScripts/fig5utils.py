...
def compare_with_prevent_30yr(model, pce_df, ascvd_indices=[111,112,113,114,115,116]):
    """
    Compare 30-year predictions using single timepoint prediction
    """
    our_30yr_risks = []
    actual_30yr = []
    ...
    # ADD DEBUGGING BEFORE THE LOOP
    print(f"Total patients in pce_df: {len(pce_df)}")
    print(f"Patients with valid prevent_impute: {pce_df['prevent_impute'].notna().sum()}")
    print(f"Patients with missing prevent_impute: {pce_df['prevent_impute'].isna().sum()}")
    ...
    for patient_idx, row in enumerate(pce_df.itertuples()):
        ...
        # Convert to 10-year risk
        risk = 1 - (1 - yearly_risk)
        our_30yr_risks.append(risk)
        
        # Still look at actual events over 10 years
        Y_ascvd = model.Y[patient_idx, ascvd_indices, enroll_time:enroll_time+30]
        actual = torch.any(torch.any(Y_ascvd, dim=0))
        actual_30yr.append(actual.item())
   
    # Rest of the function remains the same
    our_30yr_risks = np.array(our_30yr_risks)
    actual_30yr = np.array(actual_30yr)
    pce_risks = pce_df['prevent_impute'].values[:len(our_30yr_risks)]
    
    # Calculate ROC AUCs
    our_auc = roc_auc_score(actual_30yr, our_30yr_risks)
    pce_auc = roc_auc_score(actual_30yr, pce_risks)
    
    # ADD DEBUGGING INFO
    n_patients = len(actual_30yr)
    n_events = np.sum(actual_30yr)
    event_rate = (n_events / n_patients) * 100
    
    print(f"\nDEBUG INFO:")
    print(f"Number of patients: {n_patients}")
    print(f"Number of 30-year events: {n_events}")
    print(f"30-year event rate: {event_rate:.1f}%")
    print(f"Risk score range: {np.min(our_30yr_risks):.4f} to {np.max(our_30yr_risks):.4f}")
    
    print(f"\nROC AUC Comparison (30-year prediction from enrollment):")
    print(f"Our model: {our_auc:.3f}")
    print(f"PREVENT: {pce_auc:.3f}")
    
    plt.figure(figsize=(8,6))
    plot_roc_curve(actual_30yr, our_30yr_risks, label=f'Our Model (AUC={our_auc:.3f})')
    plot_roc_curve(actual_30yr, pce_risks, label=f'PREVENT (AUC={pce_auc:.3f})')
    plt.plot([0,1], [0,1], 'k--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curves for 30-year ASCVD Prediction')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def evaluate_major_diseases_wsex_with_bootstrap_from_pi(pi, Y_100k, E_100k, disease_names, pce_df, n_bootstraps=100, follow_up_duration_years=10):
    """
    Evaluate static predictions (1-year score at enrollment for N-year outcomes) using pre-computed pi tensor, with bootstrap CIs for AUC.
    Uses exact same time logic and event counting as evaluate_major_diseases_wsex_with_bootstrap.
    Also prints sex-stratified AUCs (except for sex-specific diseases) and ASCVD AUCs for patients with pre-existing RA or breast cancer.
    
    This is a faster version that accepts pre-computed pi instead of computing from model.
    
    Parameters:
    -----------
    pi : torch.Tensor
        Pre-computed pi tensor [N, D, T]. Must match the batch size of Y_100k, E_100k, pce_df.
    Y_100k : torch.Tensor
        Outcome tensor [N, D, T]
    E_100k : torch.Tensor
        Enrollment tensor [N, T]
    disease_names : list
        List of disease names
    pce_df : pd.DataFrame
        DataFrame with patient info (must have 'Sex' and 'age' columns)
    n_bootstraps : int
        Number of bootstrap iterations for CI
    follow_up_duration_years : int
        Follow-up duration (default 10 years)
    """
...
def evaluate_major_diseases_wsex_with_bootstrap_dynamic_from_pi(pi, Y_100k, E_100k, disease_names, pce_df, n_bootstraps=100, follow_up_duration_years=10, patient_indices=None):
    """
    Evaluate dynamic 10-year risk for each patient using pre-computed pi tensor, with bootstrap CIs for AUC.
    For each patient, at each year after enrollment, use the pi values to get the risk for that year.
    The cumulative 10-year risk is 1 - prod(1 - yearly_risks).
    If patient_indices is provided, subset all data to those indices.
    ...
    follow_up_duration_years : int
        Follow-up duration (default 10 years)
    patient_indices : optional
        Indices to subset data
    """
    import numpy as np
    import torch
    import pandas as pd
...
def evaluate_major_diseases_wsex_with_bootstrap_dynamic_from_pi_variable_followup(pi, Y_100k, E_100k, disease_names, pce_df, n_bootstraps=100, follow_up_duration_years=10, patient_indices=None):
    """
    Evaluate dynamic risk for each patient using pre-computed pi tensor, with bootstrap CIs for AUC.
    VARIANT: Uses variable follow-up - if patient doesn't have full requested follow-up, uses maximum available.
    
    For each patient, at each year after enrollment, use the pi values to get the risk for that year.
    The cumulative risk is 1 - prod(1 - yearly_risks) over available follow-up period.
    If patient_indices is provided, subset all data to those indices.
    
    Key difference from original: Instead of skipping patients without full follow-up, uses maximum available
    follow-up (min of requested duration and available data).
    ...
    n_bootstraps : int
        Number of bootstrap iterations for CI
    follow_up_duration_years : int
        Requested follow-up duration (default 10 years). Will use min of this and available data.
    patient_indices : optional
        Indices to subset data
    """
    import numpy as np
    ...
    for disease_group, disease_list in major_diseases.items():
        print(f"\nEvaluating {disease_group} (Dynamic {follow_up_duration_years}-Year Risk, Variable Follow-up)...")
        disease_indices = []
        unique_indices = set()
        for disease in disease_list:
            indices = [i for i, name in enumerate(disease_names) if disease.lower() in name.lower()]
        ...
        if len(int_indices_pce) == 0:
            ...
            risks_auc = np.zeros(current_N_auc)
            outcomes_auc = np.zeros(current_N_auc)
            follow_up_lengths = []  # Track follow-up length for each processed patient
            processed_indices_auc_final = []
            n_prevalent_excluded = 0
            n_skipped_no_followup = 0
            for i in range(current_N_auc):
                age = current_pce_df_auc.iloc[i]['age']
                t_enroll = int(age - 30)
                # Skip if enrollment time is invalid
                if t_enroll < 0:
                    continue
                
                # Calculate available follow-up (min of requested and available)
                available_follow_up = min(follow_up_duration_years, current_pi_auc.shape[2] - 1 - t_enroll)
                
                # Skip if no follow-up available
                if available_follow_up <= 0:
                    n_skipped_no_followup += 1
                    continue
                ...
                # Collect yearly risks for years 1 to available_follow_up after enrollment
                yearly_risks = []
                for t in range(1, available_follow_up + 1):
                    pi_diseases = current_pi_auc[i, disease_indices, t_enroll + t]
                    yearly_risk = 1 - torch.prod(1 - pi_diseases)
                    yearly_risks.append(yearly_risk.item())
                
                # Compute cumulative risk over available follow-up period
                survival_prob = np.prod([1 - r for r in yearly_risks])
                cumulative_risk = 1 - survival_prob
                risks_auc[i] = cumulative_risk
                
                # Track follow-up length for this patient
                follow_up_lengths.append(available_follow_up)
                
                # Outcome: did any event occur in the available follow-up period after enrollment?
                end_time = t_enroll + available_follow_up
                event_found = False
                for d_idx in disease_indices:
                    if d_idx >= current_Y_100k_auc.shape[1]: continue
                    if torch.any(current_Y_100k_auc[i, d_idx, t_enroll+1:end_time+1] > 0):
                        outcomes_auc[i] = 1
                        event_found = True
                        break
                processed_indices_auc_final.append(i)
            ...
            if not processed_indices_auc_final:
                ...
                if follow_up_lengths:
                    follow_up_array = np.array(follow_up_lengths)
                    follow_up_stats = {
                        'mean': float(np.mean(follow_up_array)),
                        'median': float(np.median(follow_up_array)),
                        'min': float(np.min(follow_up_array)),
                        'max': float(np.max(follow_up_array)),
                        'p25': float(np.percentile(follow_up_array, 25)),
                        'p75': float(np.percentile(follow_up_array, 75)),
                        'p10': float(np.percentile(follow_up_array, 10)),
                        'p90': float(np.percentile(follow_up_array, 90))
                    }
                else:
                    follow_up_stats = {
                        'mean': np.nan, 'median': np.nan, 'min': np.nan, 'max': np.nan,
        ...
        print(f"AUC: {auc_score:.3f} ({ci_lower:.3f}-{ci_upper:.3f}) (calculated on {n_processed} individuals)")
        print(f"Events ({follow_up_duration_years}-Year in Eval Cohort): {n_events} ({event_rate:.1f}%) (from {n_processed} individuals)")
        if follow_up_stats and not np.isnan(follow_up_stats['mean']):
            print(f"Follow-up length: mean={follow_up_stats['mean']:.1f}yr, median={follow_up_stats['median']:.1f}yr, "
                  f"range=[{follow_up_stats['min']:.1f}, {follow_up_stats['max']:.1f}]yr")
        print(f"Excluded {n_prevalent_excluded} prevalent cases for {disease_group}.")
        if n_skipped_no_followup > 0:
            print(f"Skipped {n_skipped_no_followup} patients with no follow-up available.")
    
    print(f"\nSummary of Results (Dynamic {follow_up_duration_years}-Year Risk, Variable Follow-up, Sex-Adjusted):")
    print("-" * 80)
    print(f"{'Disease Group':<20} {'AUC':<25} {'Events':<10} {'Rate (%)':<10}")
    print("-" * 80)
    for group, res in results.items():
...
def evaluate_major_diseases_wsex_with_bootstrap_dynamic_1year(pi, Y_100k, E_100k, disease_names, pce_df, n_bootstraps=100, follow_up_duration_years=1, patient_indices=None):
    """
...
def evaluate_major_diseases_wsex_with_bootstrap_dynamic_withwashout(model, Y_100k, E_100k, disease_names, pce_df, washout_years=1, n_bootstraps=100, follow_up_duration_years=10, patient_indices=None):
    """
    Evaluate dynamic 10-year risk with washout period.
    
    Same as evaluate_major_diseases_wsex_with_bootstrap_dynamic, but excludes events
    in the first washout_years from being counted as outcomes. Still predicts 10-year
    risk from enrollment, but only counts events that occur after the washout period.
    
    Parameters:
    - model: trained model
    - Y_100k: disease status matrix
    - E_100k: event times matrix
    - disease_names: list of disease names
    - pce_df: DataFrame with patient characteristics including 'age' and 'Sex'
    - washout_years: Number of years to exclude from outcome (default: 1 year)
    - n_bootstraps: Number of bootstrap samples for CI calculation
    - follow_up_duration_years: Total prediction window (default: 10 years)
    - patient_indices: Optional indices to subset the data
    """
    import numpy as np
    import torch
    ...
    for disease_group, disease_list in major_diseases.items():
        print(f"\nEvaluating {disease_group} (Dynamic {follow_up_duration_years}-Year Risk with {washout_years}-Year Washout)...")
        disease_indices = []
        unique_indices = set()
        for disease in disease_list:
            indices = [i for i, name in enumerate(disease_names) if disease.lower() in name.lower()]
        ...
        if len(int_indices_pce) == 0:
            ...
            for i in range(current_N_auc):
                ...
                ten_year_risk = 1 - survival_prob
                risks_auc[i] = ten_year_risk
                # Outcome: did any event occur in the 10 years after enrollment, EXCLUDING the washout period?
                end_time = min(t_enroll + follow_up_duration_years, current_Y_100k_auc.shape[2])
                washout_start = t_enroll + washout_years
                event_found = False
                for d_idx in disease_indices:
                    if d_idx >= current_Y_100k_auc.shape[1]: continue
                    # Only check for events AFTER the washout period
                    if washout_start < end_time:
                        if torch.any(current_Y_100k_auc[i, d_idx, washout_start:end_time] > 0):
                            outcomes_auc[i] = 1
                            event_found = True
                            break
                processed_indices_auc_final.append(i)
        ...
        print(f"AUC: {auc_score:.3f} ({ci_lower:.3f}-{ci_upper:.3f}) (calculated on {n_processed} individuals)")
        print(f"Events ({follow_up_duration_years}-Year in Eval Cohort, excluding first {washout_years} years): {n_events} ({event_rate:.1f}%) (from {n_processed} individuals)")
        print(f"Excluded {n_prevalent_excluded} prevalent cases for {disease_group}.")
    print(f"\nSummary of Results (Dynamic {follow_up_duration_years}-Year Risk with {washout_years}-Year Washout, Sex-Adjusted):")
    print("-" * 80)
    print(f"{'Disease Group':<20} {'AUC':<25} {'Events':<10} {'Rate (%)':<10}")
    print("-" * 80)
    for group, res in results.items():
...
def evaluate_major_diseases_wsex_with_bootstrap_dynamic_withwashout_from_pi(pi, Y_100k, E_100k, disease_names, pce_df, washout_years=1, n_bootstraps=100, follow_up_duration_years=10, patient_indices=None):
    """
    Evaluate dynamic risk with washout period using pre-computed pi tensor.
    
    Same as evaluate_major_diseases_wsex_with_bootstrap_dynamic_withwashout, but accepts
    pre-computed pi tensor directly instead of a model.
    
    Parameters:
    - pi: Pre-computed pi tensor of shape [N, D, T]
    - Y_100k: disease status matrix
    - E_100k: event times matrix
    - disease_names: list of disease names
    - pce_df: DataFrame with patient characteristics including 'age' and 'Sex'
            ...
            for i in range(current_N_auc):
                ...
                if len(disease_indices) == 1:
                    ...
                    if prevalent:
                        n_prevalent_excluded += 1
                        continue
                # Collect yearly risks for years 1 to follow_up_duration_years after enrollment
                yearly_risks = []
                for t in range(1, follow_up_duration_years + 1):
                    pi_diseases = current_pi_auc[i, disease_indices, t_enroll + t]
                    yearly_risk = 1 - torch.prod(1 - pi_diseases)
                ...
                cumulative_risk = 1 - survival_prob
                risks_auc[i] = cumulative_risk
                # Outcome: did any event occur in the follow_up_duration_years after enrollment, EXCLUDING the washout period?
                end_time = min(t_enroll + follow_up_duration_years, current_Y_100k_auc.shape[2])
                washout_start = t_enroll + washout_years
                event_found = False
                for d_idx in disease_indices:
...
def evaluate_major_diseases_wsex_with_bootstrap_withwashout(model, Y_100k, E_100k, disease_names, pce_df, washout_years=1, n_bootstraps=100, follow_up_duration_years=10):
    """
    Same as evaluate_major_diseases_wsex but adds bootstrap CIs for AUC.
    Uses exact same time logic and event counting as original.
    Also prints sex-stratified AUCs (except for sex-specific diseases) and ASCVD AUCs for patients with pre-existing RA or breast cancer.
...
def evaluate_major_diseases_wsex_with_bootstrap_withwashout_from_pi(pi, Y_100k, E_100k, disease_names, pce_df, washout_years=1, n_bootstraps=100, follow_up_duration_years=10):
    """
    Same as evaluate_major_diseases_wsex_with_bootstrap_withwashout but accepts pre-computed pi tensor.
    Uses exact same time logic and event counting as original.
    Also prints sex-stratified AUCs (except for sex-specific diseases) and ASCVD AUCs for patients with pre-existing RA or breast cancer.
