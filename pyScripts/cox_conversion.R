...
FH_processed=read.csv('/Users/sarahurbut/Dropbox/baselinagefamh.csv')
Y_train=readRDS("Y_train_tensor.rds")
#Y_test=readRDS("big_stuff/ukb_params.rds")$Y
disease_names=readRDS("ukb_model.rds")$disease_names[,1]
...
major_diseases <- list(
  ...
  COPD = c("Chronic airway obstruction", "Emphysema", "Obstructive chronic bronchitis"),
  Osteoporosis = c("Osteoporosis NOS"),
  Anemia = c("Iron deficiency anemias, unspecified or not due to blood loss", "Other anemias"),
  Colorectal_Cancer = c("Colon cancer", "Malignant neoplasm of rectum, rectosigmoid junction, and anus"),
...
fit_cox_baseline_models <- function(Y_train, FH_processed, train_indices,
                                    disease_mapping, major_diseases,
                                    disease_names, follow_up_duration_years = 10) {
  fitted_models <- list()
  ...
  for (disease_group in names(major_diseases)) {
    ...
    if (length(fh_cols) == 0) cat(sprintf(" - %s: No FH columns, fitting Sex only.\n", disease_group))
    ...
    if (nrow(cox_data) == 0 || sum(cox_data$event) < 5) {
      cat(sprintf("   Warning: Too few events (%d) for %s\n", sum(cox_data$event), disease_group))
      fitted_models[[disease_group]] <- NULL
      next
    }
    # Fit Cox model
    formula_str <- "Surv(age, event) ~ sex"
    if ("fh" %in% colnames(cox_data)) formula_str <- "Surv(age, event) ~ sex + fh"
    if (!is.na(target_sex_code)) {
      formula_str <- if ("fh" %in% colnames(cox_data)) "Surv(age, event) ~ fh" else "Surv(age, event) ~ 1"
    }
    fit <- try(coxph(as.formula(formula_str), data=cox_data), silent=TRUE)
    if (inherits(fit, "try-error")) {
      cat(sprintf("   Error fitting %s: %s\n", disease_group, fit))
      fitted_models[[disease_group]] <- NULL
    } else {
      fitted_models[[disease_group]] <- fit
      cat(sprintf("   Model fitted for %s using %d samples.\n", disease_group, nrow(cox_data)))
    }
  }
...
evaluate_cox_baseline_models <- function(fitted_models, Y_test, FH_test, disease_mapping, major_diseases, disease_names, follow_up_duration_years = 10) {
  test_results <- list()
  ...
  for (disease_group in names(fitted_models)) {
    ...
    if (is.null(model)) {
      test_results[[disease_group]] <- list(c_index=NA, ci=c(NA, NA), n_events=0, n_total=0)
      next
    }
    ...
    if (nrow(current_FH_test) == 0) {
      cat(sprintf("   Warning: No individuals for target sex code %s.\n", target_sex_code))
      test_results[[disease_group]] <- list(c_index=NA, ci=c(NA, NA), n_events=0, n_total=0)
      next
    }
    disease_indices <- unlist(lapply(major_diseases[[disease_group]], function(disease) {
