...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit_psitest(nn.Module):
    ...
    def warp_time(self, t, rho_ik, T):
        # t: [T] (time points)
        # rho_ik: scalar or [N, K_total]
        t_norm = t / T
...
def torch_linear_interp(x, xp, fp):
    ...
    x: points to interpolate at (any shape)
    xp: known x (1D, sorted, shape [T])
    fp: known y (same shape as xp, [T])
    Returns: interpolated values at x (same shape as x)
    """
    # Clamp x to the valid range
    x = x.clamp(xp[0], xp[-1])
    # Find indices of the left bin
...
def vectorized_linear_interp(fp, x):
    # fp: [B, T] (values to interpolate)
    # x: [B, Tq] (query points, float, in [0, T-1])
    T = fp.shape[1]
    x0 = torch.floor(x).long().clamp(0, T-2)
    x1 = x0 + 1
    w = x - x0.float()
    fp0 = torch.gather(fp, 1, x0)
