...
def plot_pi_heatmap(pi_avg, psi, clusters, disease_names, age_offset=30,
    ...
    ax.set_yticklabels([f'{disease_order_by_sig[i]}' for i in y_tick_indices], fontsize=10)
...
def main():
    ...
    # Load pi (theta) - average probability over people
    ...
    print(f"Pi tensor shape: {pi_full.shape}")  # Should be (N, D, T) or (N, K, T)
    
    # Average over people (patients) to get (D, T) - average probability for each disease at each age
    # If pi_full is (N, K, T), we need to aggregate over signatures first
    if len(pi_full.shape) == 3:
        if pi_full.shape[1] > 50:  # Likely (N, D, T) - direct disease probabilities
        ...
        else:  # Likely (N, K, T) - signature probabilities, need to convert to disease probabilities
            ...
            # This would require phi or clusters to convert - for now assume (N, D, T)
            pi_avg = np.mean(pi_full, axis=0)  # Shape: (D, T)
    else:
    ...
    # Load UKB checkpoint for psi, clusters, and disease names
