#!/usr/bin/env python3
"""
Verify that pi_enroll_fixedphi_sex_FULL.pt matches concatenation of batch files.

Checks:
1. Load pi_enroll_fixedphi_sex_FULL.pt from enrollment_predictions_fixedphi_correctedE_vectorized/
2. Load all batch files from same directory
3. Concatenate batches in order (0-10000, 10000-20000, ..., 390000-400000)
4. Compare concatenated tensor with FULL file

Uses sampling approach for faster verification.
"""
...
def load_and_assemble_batches(batch_dir, max_patients=400000, batch_size=10000):
    ...
    # Find all batch files (excluding FULL)
    batch_files = [f for f in batch_dir.glob('pi_enroll*.pt') if 'FULL' not in f.name]
...
def load_pi_full(pi_full_path):
    """Load pi_enroll_fixedphi_sex_FULL.pt and handle different formats."""
    pi_full_path = Path(pi_full_path)
    
    if not pi_full_path.exists():
        raise FileNotFoundError(f"FULL file not found: {pi_full_path}")
    
    print(f"\nLoading FULL file: {pi_full_path}")
...
def main():
    batch_dir = Path('/Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_correctedE_vectorized/')
    pi_full_path = batch_dir / 'pi_enroll_fixedphi_sex_FULL.pt'
    
    print("="*80)
    print("VERIFYING pi_enroll_fixedphi_sex_FULL.pt ASSEMBLY")
    print("="*80)
    print(f"FULL file: {pi_full_path}")
    ...
    print(f"Assembled shape: {pi_assembled.shape}")
    print(f"FULL file shape: {pi_full.shape}")
    print(f"Max difference: {max_diff:.2e}")
    print(f"Mean difference: {mean_diff:.2e}")
    
    if is_equal:
        print("\n✅ VERIFICATION PASSED: FULL file matches concatenated batches!")
    else:
        print("\n❌ VERIFICATION FAILED: FULL file does NOT match concatenated batches!")
        print("\nPossible reasons:")
        print("  1. FULL file was created from different batch files")
