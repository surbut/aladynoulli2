...
def generate_age_stratified_age_offset_predictions(approach='pooled_retrospective', max_offset=9, start_idx=0, end_idx=10000):
    ...
    if approach == 'pooled_retrospective':
        # Files from corrected E run (local vectorized)
        ...
        # Files from local run (old path, kept for backward compatibility)
    ...
    output_dir = script_dir.parent / 'results' / 'age_offset' / approach / 'age_stratified'
