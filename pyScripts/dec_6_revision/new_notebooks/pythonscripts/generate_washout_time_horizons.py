...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/sarahurbut/aladynoulli2/pyScripts/dec_6_revision/new_notebooks/results/washout_time_horizons/',
    ...
    # Set N to 400K (standard size for pi_enroll_fixedphi_sex_FULL.pt)
    N = 400000
    ...
    print("="*80)
    print(f"\nNOTE: Processing {N:,} patients with {args.n_bootstraps} bootstraps.")
    print(f"      For 30-year horizon, this processes ~{N:,} patients × 30 years = ~{N*30:,} risk calculations.")
    print(f"      This will take time - progress will be shown as each disease is evaluated.")
    print("="*80)
    ...
    print(f"\nWill process {N:,} patients (standard size for /Users/sarahurbut/Library/CloudStorage/Dropbox/enrollment_predictions_fixedphi_correctedE_vectorized/pi_enroll_fixedphi_sex_FULL.pt)")
    ...
    for horizon_years in [10, 30]:
        ...
        print(f"  Data shapes: pi={pi_full.shape}, Y={Y_full.shape}, E={E_full.shape}")
        
        print(f"\nEvaluating dynamic {horizon_years}-year predictions with {args.washout_years}-year washout...")
        print(f"  This may take a while for {horizon_years}-year horizon with {N:,} patients...")
        import time
    ...
    if static_output_file.exists():
        ...
        print(f"\nEvaluating static 10-year predictions (1-year score) with {args.washout_years}-year washout...")
        import time
        start_time = time.time()
