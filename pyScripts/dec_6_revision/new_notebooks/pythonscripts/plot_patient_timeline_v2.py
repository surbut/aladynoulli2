#!/usr/bin/env python3
"""
Plot Patient Timeline V2 - Enhanced Individual Trajectory Visualization

Creates a multi-panel plot showing:
- Panel 1: Signature loadings (θ) vs Age with stacked average theta bar
- Panel 2: Disease timeline with readable disease labels  
...
def plot_patient_timeline(patient_idx, 
    ...
    # ============================================================================
    # Create Plot with optimized layout
    # ============================================================================

    fig = plt.figure(figsize=figsize, facecolor='white')
    
    # Create a complex grid layout
    # Top panel: Signatures (full width)
    # Middle: Disease timeline (left 2/3) + Disease legend (right 1/3)  
    # Bottom: Probabilities (full width)
    gs = plt.GridSpec(3, 3, width_ratios=[2, 2, 1.5], height_ratios=[2.5, 2, 2.5], 
    ...
    # Plot signature trajectories with thicker lines for better visibility
    for k in range(K_total):
        ax1.plot(ages, patient_theta[k, :], 
                 label=f'Signature {k}', linewidth=2.5, color=colors[k], alpha=0.85)
    ...
    ax1_bar = ax1.inset_axes([0.82, 0.65, 0.15, 0.3])
    ...
    # ============================================================================
    # Panel 2: Disease timeline - left 2/3 of middle row
    # ============================================================================
    ...
    if len(diagnosis_times) > 0:
        ...
        # Limit to first 25 diseases for readability
        max_diseases_shown = 25
        ...
        for i, (d, t_diag) in enumerate(diag_order_shown):
            ...
            ax2.scatter(age_at_diag, y_pos, s=100, color=color, alpha=0.85, zorder=10, 
                       edgecolors='black', linewidths=1.2)
            
            # Add disease number on the left
            ax2.text(29.5, y_pos, f'{i+1}', fontsize=9, fontweight='bold', 
    ...
    # ============================================================================
    # Panel 2b: Disease legend - right 1/3 of middle row
    # ============================================================================
    ax2_legend = fig.add_subplot(gs[1, 2])
    ax2_legend.axis('off')
    
    if len(diagnosis_times) > 0:
        # Create scrollable disease list
        legend_text = []
        for i, (d, t_diag) in enumerate(diag_order_shown):
            disease_name = disease_names[d] if d < len(disease_names) else f'Disease {d}'
            sig_for_disease = initial_clusters[d] if d < len(initial_clusters) else -1
            
            # Smart truncation of disease names
            max_len = 28
            ...
            legend_text.append(f'{i+1:2d}. {disease_name[:28]:<28s}\n     Sig {sig_for_disease:2d}, Age {age_at_diag:2d}')
        
        # Display disease legend with better formatting
        ax2_legend.text(0.05, 0.98, 'Disease Details:', fontsize=11, fontweight='bold',
                       transform=ax2_legend.transAxes, va='top')
        
        # Use monospace font for alignment
        legend_str = '\n'.join(legend_text)
        ax2_legend.text(0.05, 0.92, legend_str, fontsize=7.5,
                       transform=ax2_legend.transAxes, va='top', 
...
if __name__ == "__main__":
    ...
    output_path = f'/Users/sarahurbut/aladynoulli2/patient_{patient_idx}_timeline_v2.pdf'
    
    fig = plot_patient_timeline(
        patient_idx=patient_idx,
        output_path=output_path,
        figsize=(18, 14)  # Wider format for better readability
    )
