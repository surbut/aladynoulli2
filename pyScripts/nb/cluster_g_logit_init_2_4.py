...
class AladynSurvivalFixedKernelsAvgLoss_clust_logitInit24(nn.Module):
    ...
    def initialize_params(self, true_psi=None, **kwargs):
        ...
        if true_psi is not None:
            ...
            for k in range(self.K):
                ...
                psi_init[k, cluster_mask] = 2.0 + 0.1 * torch.randn(cluster_mask.sum())
                psi_init[k, ~cluster_mask] = -4.0 + 0.01 * torch.randn((~cluster_mask).sum())
