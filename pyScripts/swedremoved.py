...
def bayesian_meta_analysis(studies_dict, prior_mean=0, prior_sd=0.3, title="Meta-Analysis", weighting='inverse_variance'):
    """
    ...
    for name, data in studies_dict.items():
        ...
        if weighting == 'inverse_variance':
            ...
            weights.append(1)  # equal weighting
    ...
    if weighting == 'inverse_variance':
        ...
        # For equal weighting, use average of variances
        pooled_se = np.sqrt(np.mean(ses**2) / len(ses))
...
# Create studies dataset excluding the problematic Swedish study
studies_no_swedish = {k: v for k, v in studies.items() if k != 'Ahlqvist_2024'}

print("COMPARISON: WITH vs WITHOUT PROBLEMATIC SWEDISH STUDY")
print("="*70)

for prior_name, prior_params in priors.items():
    print(f"\n{prior_name.upper()} PRIOR")
    print("-" * 30)
    
    # With Swedish study (inverse variance weighted - dominated by huge N)
    result_with = bayesian_meta_analysis(
        studies, 
    ...
    # Without Swedish study 
    result_without = bayesian_meta_analysis(
        studies_no_swedish, 
        prior_mean=prior_params['mean'], 
        prior_sd=prior_params['sd'],
        weighting='inverse_variance'
    ...
    # Equal weighting (all studies count equally regardless of sample size)
    result_equal = bayesian_meta_analysis(
        studies, 
        prior_mean=prior_params['mean'], 
        prior_sd=prior_params['sd'],
        weighting='equal'
    ...
    print(f"WITH Swedish study (IV weighted):    OR: {result_with['posterior_or']:.2f} ({result_with['posterior_ci'][0]:.2f}-{result_with['posterior_ci'][1]:.2f})")
    print(f"WITHOUT Swedish study:              OR: {result_without['posterior_or']:.2f} ({result_without['posterior_ci'][0]:.2f}-{result_without['posterior_ci'][1]:.2f})")
    print(f"Equal weighting (all studies):      OR: {result_equal['posterior_or']:.2f} ({result_equal['posterior_ci'][0]:.2f}-{result_equal['posterior_ci'][1]:.2f})")
    print(f"P(meaningful harm) without Swedish: {result_without['prob_meaningful']:.1%}")

print(f"\nSibling Studies (family-controlled, skeptical prior):")
...
print(f"  OR: {sib_result['posterior_or']:.2f} ({sib_result['posterior_ci'][0]:.2f}-{sib_result['posterior_ci'][1]:.2f})")
print(f"  P(any harm): {sib_result['prob_harmful']:.1%}")

print("\n" + "="*70)
print("WHY THE SWEDISH STUDY IS PROBLEMATIC:")
print("="*70)
print("• Sample size: 2.48 MILLION (dominates all other studies combined)")
print("• Reported acetaminophen use: 7.5% (implausibly low vs ~50% expected)")
print("• Suggests massive exposure misclassification")
print("• Gets ~99% of the weight in inverse-variance meta-analysis")
print("• May be measuring something different than other studies")

print("\nWEIGHTS IN INVERSE-VARIANCE META-ANALYSIS:")
total_weight = sum(1/data['se']**2 for data in studies.values())
for name, data in studies.items():
    weight = (1/data['se']**2) / total_weight
    print(f"{name}: {weight:.1%} weight (N={data['n']:,})")
