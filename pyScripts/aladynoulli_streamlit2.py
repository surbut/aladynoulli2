...
class ModelVisualizer:
    ...
    def plot_signature_dynamics_with_diagnoses(self, person_idx, Y=None, show_population=False, highlight_cycles=False, time_idx=None):
        if time_idx is None:
            time_idx = self.T - 1  # Show all by default
        theta = self.theta[person_idx, :, :time_idx+1]
        time_points = np.arange(time_idx+1)
        colors = sns.color_palette("tab20", self.K)
        fig = plt.figure(figsize=(12, 8))
        gs = gridspec.GridSpec(3, 2, height_ratios=[2.5, 1, 1.2], width_ratios=[4, 0.8], hspace=0.15)
        ax1 = fig.add_subplot(gs[0, 0])  # Top left: signature trajectories
        ax2 = fig.add_subplot(gs[1, 0], sharex=ax1)  # Middle left: diagnosis timeline
        ax3 = fig.add_subplot(gs[:, 1])  # Right: stacked bar
        ax4 = fig.add_subplot(gs[2, 0], sharex=ax1)  # Bottom left: prediction changes

        # --- Plot signature trajectories ---
        for k in range(self.K):
            ax1.plot(time_points, theta[k, :], color=colors[k], label=f"Signature {k}", linewidth=2)
        if show_population:
            mean_theta = np.mean(self.theta, axis=0)  # (K, T)
            for k in range(self.K):
                ax1.plot(time_points, mean_theta[k, :time_idx+1], color=colors[k], linestyle='--', linewidth=1.5, alpha=0.7)
        ax1.set_title(f"Signature Loadings (θ) Over Time\nPerson {person_idx}")
        ax1.set_ylabel("Signature Loading (θ)")
        ax1.grid(True, axis='y', linestyle='--', alpha=0.5)
        ax1.set_ylim(bottom=0)

        # Optionally highlight cyclical patterns
        if highlight_cycles:
            from scipy.signal import find_peaks
            for k in range(self.K):
                peaks, _ = find_peaks(theta[k, :], distance=5)
                ax1.scatter(time_points[peaks], theta[k, peaks], color=colors[k], marker='o', s=40, zorder=10)

        # --- Plot diagnosis events and highlight background ---
        diagnosis_times = []
        if Y is not None:
            diagnoses = Y[person_idx, :, :time_idx+1]
            for d in range(self.D):
                diag_times = np.where(diagnoses[d, :] > 0.5)[0]
                if len(diag_times) > 0:
                    t_diag = diag_times[0]
                    diagnosis_times.append(t_diag)
                    sig_idx = np.argmax(self.psi[:, d])
                    color = colors[sig_idx]
                    ax1.axvline(x=t_diag, color=color, linestyle=':', alpha=0.5, linewidth=1)
                    ax2.scatter(t_diag, d, color=color, s=40, edgecolors='black', linewidth=0.5)
            # Highlight background for new diagnoses
            diagnosis_times = sorted(set(diagnosis_times))
            for t_diag in diagnosis_times:
                ax1.axvspan(t_diag-0.5, t_diag+0.5, color='orange', alpha=0.12, zorder=0)
            ax2.set_yticks(range(self.D))
            ax2.set_yticklabels(self.disease_names, fontsize='small')
            ax2.set_ylim(-0.5, self.D - 0.5)
            ax2.invert_yaxis()
            ax2.set_xlabel("Time")
        ...
        bottom = 0
        for k in range(self.K):
            ax3.bar([0], [theta_avg[k]], bottom=bottom, color=colors[k], alpha=0.7, width=0.3)
            bottom += theta_avg[k]
        ...
        # --- Show model's predictions for next diagnosis ---
        if Y is not None:
            # Find the next diagnosis after each time t
            pi_t = self.compute_disease_probabilities(person_idx)[:time_idx+1]  # (t, D)
            # For each t, find the next diagnosis (if any)
            next_diag_prob = np.full(time_idx+1, np.nan)
            for t in range(time_idx+1):
                if t+1 < self.T:
                    arr = Y[person_idx, :, t+1:] > 0.5
                    try:
                        if arr.ndim == 2 and arr.shape[1] > 0:
                            diag_mask = np.any(arr, axis=1)
                        elif arr.ndim == 1 and arr.shape[0] == self.D:
                            diag_mask = arr
                        else:
                            diag_mask = np.zeros(self.D, dtype=bool)
                    except Exception as e:
                        print(f"Shape error at t={t}: arr.shape={arr.shape}, arr.ndim={arr.ndim}, error={e}")
                        diag_mask = np.zeros(self.D, dtype=bool)
                else:
                    diag_mask = np.zeros(self.D, dtype=bool)
                if np.any(diag_mask):
                    next_diag_idx = np.argmax(diag_mask)
                    next_diag_prob[t] = pi_t[t, next_diag_idx]
            ax4.plot(time_points, next_diag_prob, color='black', label="Predicted Prob. of Next Diagnosis")
            ax4.set_ylabel("Pred. Prob. Next Dx")
            ax4.set_xlabel("Time")
            ax4.set_title("Model's Prediction for Next Diagnosis")
...
def main():
    ...
    with tab1:
        Y = first_model['Y'] if 'Y' in first_model else None
        fig = visualizer.plot_signature_dynamics_with_diagnoses(
        ...
        st.sidebar.markdown("""
        ### How to interpret:
        - **Top Left**: Signature trajectories for the selected person (solid lines)
        - **Dashed lines**: Population mean (if enabled)
        - **Vertical dotted lines**: Diagnosis events
        - **Bottom Left**: Timeline of diagnoses
        - **Right**: Stacked bar summary of average signature loadings
        """)
