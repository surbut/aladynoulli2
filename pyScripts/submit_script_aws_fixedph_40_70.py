...
class SurvivalModelTrainer:
    ...
    def load_model_essentials(self):
        ...
        try:
            # Load large matrices
            Y = torch.load(os.path.join(self.base_data_path, 'Y_tensor.pt'), weights_only=False)
            E = torch.load(os.path.join(self.base_data_path, 'E_matrix.pt'), weights_only=False)
            G = torch.load(os.path.join(self.base_data_path, 'G_matrix.pt'), weights_only=False)
            
            # Load other components
            essentials = torch.load(os.path.join(self.base_data_path, 'model_essentials.pt'), weights_only=False)
